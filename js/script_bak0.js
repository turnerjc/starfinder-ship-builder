const JSON_FILE = "app/data/ship-builder.json";
...
function maybeCreateProperty(obj, prop, type = "Object") {
...
function Ship(json) {
	...
	this.app = new Vue({
		...
		computed: {
			...
			armourClass: function() {
				return 10 +
					...
					parseInt(this.params.crewSkills.pilot.skills.piloting.ranks)
				;
			},
		...
		methods: {
			...
			getCrewSkillDescription: function(crewMember, skillName, isLastInList = false) {
				var modifier = this.params['crew' + crewMember + skillName + 'Modifier'];
				var ranks = this.params['crew' + crewMember + skillName + 'Ranks'];
				var desc = "";
				if( modifier > 0 && ranks > 0) {
					desc = skillName + this.getPrefixedModifier(modifier) +
						' (' + ranks + ' ranks)' + (isLastInList ? '' : ', ');
				}
				return desc;
			},
